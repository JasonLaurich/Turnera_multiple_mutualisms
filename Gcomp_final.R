...
morrissey_vect_cor<-read.table(file="final_ranskew.txt", sep="\t")
...
MCMCG.kr <- kr.subspace(Garray2, vec = rep(2,m))

# Save the object so that it can be looked at later.
#save(MCMCG.kr, file='MCMCG.kr.final.Rdata')
#load('MCMCG.kr.Rdata')

# Load up the table containing the results of Krzanowski's analysis fitted for the Morrissey-adjusted null matrices. 
kr_ran<-read.table('final_krzan.txt', na.strings="", header=T, sep='\t')

HPD.H.val <- cbind(HPDinterval(as.mcmc(MCMCG.kr$MCMC.H.val)), colMeans(MCMCG.kr$MCMC.H.val), HPDinterval(as.mcmc(kr_ran)), colMeans(kr_ran))
HPD.H.val
...
for (i in 1:5){
  for (a in 1:4){
    hpd<-HPDinterval(as.mcmc(angmat[,i,a]))
    anghpd[,1,i]<-hpd[1,1]
    anghpd[,2,i]<-hpd[1,2]
  }
}
...
k<-ggplot(data=HPD_K,aes(y=mean,x=H, group=G, colour=G))+geom_point(position=position_dodge(width=0.4), size=3)
k<-k + geom_errorbar(aes(ymin=min, ymax=max), position=position_dodge(width=0.4), size=1,width=0.1)
k<-k +theme(axis.title.y=element_blank(), axis.title.x=element_blank())
k<-k + ylim(0,5)
k<-k + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
k<-k + theme_bw()
#k2<-k2 + scale_y_continuous(breaks=c(0, 1, 2, 3, 4, 5))
k<-k + theme(legend.position = c(0.6,0.9))
k<-k + theme(legend.title=element_blank())
k<-k + scale_color_manual(labels=c("Observed","Randomized"),values = c("dodgerblue3","red3"))
k<-k + scale_x_discrete(labels=c(expression(h[1]),expression(h[2]), expression(h[3]), expression(h[4])))
k<-k + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
k<-k + ylab("Eigenvalue")
k<-k + xlab("Eigenvector of H")
k<-k + theme(axis.text=element_text(size=12), axis.title=element_text(size=14,face="bold"))
k<-k + theme(panel.border = element_blank())
k<-k + theme(axis.line = element_line(color = 'black'))
k<-k + theme(legend.text = element_text(size=12, face="bold"))
k<-k +theme(axis.title.y=element_blank(), axis.title.x=element_blank())

y.grob <- textGrob("Eigenvalue", gp=gpar(fontsize=15), rot=90)
x.grob <- textGrob("Eigenvector of H", gp=gpar(fontsize=15))

grid.arrange(arrangeGrob(k,left=y.grob, bottom=x.grob))
...
MCMC.covtensor <- covtensor(Garray2)

# Save the object so that it can be looked at later.
#save(MCMC.covtensor, file='MCMC.covtensor.final.Rdata')
#load('MCMC.covtensor.final.Rdata')
...
cov_ran<-read.table('final_fourth_order.txt', na.strings="", header=T, sep='\t')

nnonzero <- min(n*(n+1)/2,m2-1)
...
tens<-tens + scale_y_continuous(breaks=c(0, 0.5, 1.0, 1.5))
...
e11.proj <- apply(Garray2, 3:4, proj, b = e11)
...
e21.proj <- apply(Garray2, 3:4, proj, b = e21)
